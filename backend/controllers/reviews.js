...
const setMovieReview =async (req,res) =>{
 ...
 try {
   ...
   if(user.VIP === 0){
    ...
    await user.save();
    await movie.save()
   res.status(StatusCodes.OK).json({msg:'Movie reviews have been inserted!'})
    }catch(error){
        console.error(error);
...
const getUserMovieReview = async (req,res)=>{
    ...
    try {
      ...
      if (!userReview) {
        return res.status(StatusCodes.NOT_FOUND).json({ message: 'User review not found for this movie' });
      }
...
const updateUserMovieReview = async (req, res) => {
    ...
    try {
      ...
      res.status(StatusCodes.OK).json({ message: 'Review and rating updated successfully', updatedMovie });
    } catch (error) {
      res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: 'Internal server error' });
    }
  };
...
module.exports = {
setMovieReview,getUserMovieReview,updateUserMovieReview
}